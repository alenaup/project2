<div x-data="{ 
    ...
    edit_id: $wire.entangle('edit_id'),
    edit_tanggal_mulai: $wire.entangle('edit_tanggal_mulai'),
    edit_tanggal_selesai: $wire.entangle('edit_tanggal_selesai'),
    edit_keterangan: $wire.entangle('edit_keterangan'),
    edit_file_lama: $wire.entangle('edit_file_lama'),
    ...
    openEditModal(id, mulai, selesai, ket, fileLama) {
        this.edit_id = id;
        this.edit_tanggal_mulai = mulai;
        this.edit_tanggal_selesai = selesai;
        this.edit_keterangan = ket;
        this.edit_file_lama = fileLama;
        $wire.set('edit_file_surat', null);
        this.openEdit = true;
    }
}">
    <div class="bg-white rounded-xl shadow p-6 relative">
        ...
        @if (session('success_riwayat'))
            <div
                class="bg-emerald-50 text-emerald-600 px-4 py-3 rounded-lg text-sm font-semibold flex items-center gap-2 mb-4">
        ...
        <div class="space-y-3 relative">
            <div wire:loading.flex wire:target="filterStatus, deletePengajuan"
                class="absolute inset-0 z-10 bg-white/70 backdrop-blur-sm flex items-center justify-center">
                    ...
                    <div class="flex items-start justify-between gap-3 flex-wrap">
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-2 flex-wrap mb-1">
                                ...
                                @if ($item->status == 'menunggu')
                                    <span
                                        class="text-xs px-2 py-0.5 rounded-full font-medium bg-amber-50 text-amber-600"><i
                                            class="fa-solid fa-hourglass-half mr-1"></i> Menunggu</span>
                                @elseif($item->status == 'disetujui')
                                    <span
                                        class="text-xs px-2 py-0.5 rounded-full font-medium bg-emerald-50 text-emerald-600"><i
                                            class="fa-solid fa-circle-check mr-1"></i> Disetujui</span>
                                @else
                                    <span class="text-xs px-2 py-0.5 rounded-full font-medium bg-red-50 text-red-600"><i
                                            class="fa-solid fa-circle-xmark mr-1"></i> Ditolak</span>
                                @endif
                            </div>

                            <p class="text-gray-500 text-xs mb-2">Diajukan:
                                {{ \Carbon\Carbon::parse($item->tanggal_pengajuan)->translatedFormat('d M Y H:i') }}</p>
                            ...
                            <div class="flex items-center gap-2 mt-2">
                                ...
                                @endif
                                <span
                                    class="text-xs text-gray-500 truncate max-w-[180px]">{{ basename($item->file_surat) }}</span>

                                <button type="button" @click="openPreviewModal('{{ Storage::url($item->file_surat) }}', '{{ strtolower(pathinfo($item->file_surat, PATHINFO_EXTENSION)) === 'pdf' ? 'pdf' : 'image' }}')"
                        ...
                        @if ($item->status == 'menunggu')
                            <div class="flex gap-2 shrink-0">
                                <button @click="openEditModal({{ $item->id_perizinan }}, '{{ $item->tanggal_mulai }}', '{{ $item->tanggal_selesai }}', '{{ addslashes($item->keterangan) }}', '{{ $item->file_surat }}')"
                                ...
                                <button wire:confirm="Yakin ingin menghapus pengajuan ini?"
                                    wire:click="deletePengajuan({{ $item->id_perizinan }})"
    ...
    <div x-show="openEdit" x-cloak x-transition
        ...
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
            <div class="flex items-center justify-between px-5 py-4 border-b">
                ...
                <button @click="openEdit = false" type="button" class="text-gray-400 hover:text-gray-600 transition text-xl">
            ...
            <form wire:submit.prevent="simpanEdit">
                <div class="p-5 space-y-4 relative">
                    <div wire:loading.flex wire:target="simpanEdit, edit_file_surat"
                    ...
                    <div>
                        ...
                        <textarea x-model="edit_keterangan" rows="3"
                    ...
                    <div>
                        ...
                        @if (!$edit_file_surat)
                            <div class="flex items-center gap-3 bg-gray-50 border border-gray-200 rounded-xl p-3 mb-2">
                                <div class="text-2xl w-8 text-center"><i class="fa-solid fa-file text-gray-400"></i>
                                </div>
                                <div class="flex-1 min-w-0">
                                    <p class="text-xs font-semibold text-gray-700 truncate" x-text="edit_file_lama ? edit_file_lama.split('/').pop() : ''"></p>
                                    <p class="text-xs text-gray-400">Surat saat ini</p>
                                </div>
                                <label
                                    class="text-emerald-600 text-xs font-semibold hover:underline whitespace-nowrap cursor-pointer">
                                    <i class="fa-solid fa-arrow-up-from-bracket mr-1"></i>Ganti
                                    <input type="file" wire:model="edit_file_surat"
                                        accept=".jpg,.jpeg,.png,.pdf" class="hidden">
                                </label>
                            </div>
                        @else
                            <div
                                class="flex items-center gap-3 bg-emerald-50 border border-emerald-200 rounded-xl p-3 mb-2">
                                <div class="text-2xl w-8 text-center"><i
                                        class="fa-solid fa-file-circle-check text-emerald-500"></i></div>
                                <div class="flex-1 min-w-0">
                                    <p class="text-xs font-semibold text-gray-700 truncate">
                                        {{ $edit_file_surat->getClientOriginalName() }}</p>
                                    <p class="text-xs text-gray-400">File baru akan menggantikan surat lama</p>
                                </div>
                                <button type="button" wire:click="$set('edit_file_surat', null)"
                                    class="text-red-500 hover:text-red-700 text-sm">
                                    <i class="fa-solid fa-xmark"></i> Batal
                                </button>
                            </div>
                        @endif
                        @error('edit_file_surat')
                            <span class="text-xs text-red-500">{{ $message }}</span>
                        @enderror
                    </div>

                    <div class="flex gap-3 pt-2">
                        <button type="submit" @click="openEdit = false"
