<div>
    ...
    <div x-data="{ activeTab: @entangle('activeTab').live }"
        class="animate-bitem bg-white px-4 md:px-10 py-3 border-gray-200 border-b border-t flex gap-2 text-sm overflow-x-auto">
        <button type="button" @click="activeTab = 'admin_vendor'" :aria-pressed="activeTab === 'admin_vendor'"
            :class="activeTab === 'admin_vendor'
                ?
        ...
        <button type="button" @click="activeTab = 'hr'" :aria-pressed="activeTab === 'hr'"
            :class="activeTab === 'hr'
                ?
                'bg-green-600 border-green-600 text-white font-semibold shadow-sm' :
                'bg-white border-gray-200 text-gray-500 hover:text-green-700 hover:border-green-300 hover:bg-green-50'"
        ...
        <button type="button" @click="activeTab = 'kepala_departemen'"
            :aria-pressed="activeTab === 'kepala_departemen'"
            :class="activeTab === 'kepala_departemen'
                ?
                'bg-green-600 border-green-600 text-white font-semibold shadow-sm' :
                'bg-white border-gray-200 text-gray-500 hover:text-green-700 hover:border-green-300 hover:bg-green-50'"
            class="px-4 py-2 rounded-lg border transition-all duration-200 whitespace-nowrap">
                ...
                <tbody class="relative">
                    ...
                    @forelse($users as $index => $user)
                        <tr
                            ...
                            <td class="p-3">
                                <span
                                    class="{{ $user->status === 'active' ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600' }} px-2 py-1 rounded text-xs font-semibold">
                                    {{ ucfirst($user->status) }}
                                </span>
                            </td>
    ...
    @if ($showModal)
        <div
            class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 px-4 transition-opacity duration-300">
            <div
                class="bg-white w-full max-w-xl rounded-xl overflow-y-auto shadow-2xl transform scale-100 transition-transform duration-300">
                <!-- HEADER -->
