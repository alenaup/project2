...
@section('content')
    ...
    <div x-data="{ ...dashboard(), open: false }">
        <div class='animate-item flex flex-col md:flex-row justify-between items-start md:items-center border-b border-slate-200/60 pb-5 mb-6 gap-4 bg-gradient-to-r from-emerald-100 p-6 rounded-xl'>
            ...
            <div class='flex items-center gap-3 w-full md:w-auto' x-data="{
                isExportOpen: false,
                exportMonth: new Date().getMonth() + 1,
                exportYear: new Date().getFullYear(),
                startExport() {
                    const token = 'export_' + Date.now();
                    const url = new URL('/kepala-departement/export-jadwal', window.location.origin);
                    url.searchParams.set('month', this.exportMonth);
                    url.searchParams.set('year', this.exportYear);
                                    ...
                                    <div @click="isPastDate(days[index].date_full) ? window.dispatchEvent(new CustomEvent('flash-error', { detail: { message: 'Tidak dapat mengubah atau membuat jadwal untuk hari yang sudah lewat.' } })) : openModal(emp.id, days[index].date_full, days[index].date_full)"
                                        ...
                                        <template x-if="shift">
                                            <div class="px-2.5 py-1.5 rounded-xl text-xs text-center w-full shadow-xs transition duration-200"
                                                ...
                                                <div class="font-bold capitalize text-[10px]" x-text="typeof shift === 'object' ? shift.nama : shift"></div>
                                                <div class="text-[9px] font-medium opacity-85 mt-1">
                                                    <span x-text="typeof shift === 'object' ? (shift.nama.toLowerCase() === 'libur' ? 'Libur' : shift.jam_masuk) : (shift === 'pagi' ? '06.00' : (shift === 'siang' ? '14.00' : (shift === 'sore' ? '15.00' : (shift === 'malam' ? '22.00' : 'Libur'))))"></span>
                                                </div>
